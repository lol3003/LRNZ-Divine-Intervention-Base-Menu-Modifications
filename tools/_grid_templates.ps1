...
function Write-VanillaPerkButton {
    ...
    [void]$Sb.AppendLine("                    }")
    [void]$Sb.AppendLine("")
    # owned layer: gold tint, visible only when the perk is granted to the selected
    # dynasty (the remove SGUI's is_shown = has_dynasty_perk is exactly that state).
    [void]$Sb.AppendLine("                    background = {")
    [void]$Sb.AppendLine("                        visible = ""[GetScriptedGui('DI_perk_remove_$Key').IsShown(GuiScope.SetRoot(GetPlayer.MakeScope).End)]""")
    [void]$Sb.AppendLine("                        size = { 100% 100% }")
    [void]$Sb.AppendLine("                        texture = ""gfx/interface/component_masks/mask_frame_horizontal.dds""")
    [void]$Sb.AppendLine("                        tintcolor = { 0.85 0.7 0.1 0.85 }")
    [void]$Sb.AppendLine("                        alpha = 0.85")
    [void]$Sb.AppendLine("                    }")
    [void]$Sb.AppendLine("")
    ...
    [void]$Sb.AppendLine("                        }")
    # checkmark in the top-right corner when owned
...
function Write-VanillaTrackSection {
    ...
    # fully-owned indicator: gold when the add-all SGUI reports not-shown
    # (its is_shown = NOT(<track>_perks >= count), so not-shown = all owned)
    [void]$Sb.AppendLine("                    background = {")
